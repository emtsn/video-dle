...
function App(): React.ReactElement {
  ...
  const handleSelect = useCallback((videoId: string): void => {
    ...
    if (videoId === answer.videoId) {
      ...
      messageApi.open({
        type: 'error',
        content: 'You ran out of guesses. The correct answer was ' + vidData[answer.videoId].title + ' by ' + vidData[answer.videoId].uploaderName,
        duration: 3
      });
    }
  ...
  return (
    <div className="App">
      ...
      <Layout className='main-layout'>
        ...
        <Content style={{ paddingTop: '16px', paddingLeft: '5%', paddingRight: '5%' }}>
          <Space direction='vertical' style={{ display: 'flex' }}>
            <Row align='middle' justify='start' gutter={8}>
              <Col span='12'>
                <GuessInput vidData={Object.values(vidData)} handleSelect={handleSelect} disabled={playState !== PlayState.InProgress}></GuessInput>
              </Col>
              <Col span='8'>
                <Space>
                  {new Array(Math.max(0, MAX_GUESSES - guessedVideos.length)).fill(<HeartFilled style={{ color: 'red' }} />)}
                </Space>
              </Col>
            </Row>
        ...
        <Footer><a href='https://github.com/emtsn' style={{ color: '#828282' }}><Space><GithubOutlined />emtsn</Space></a></Footer>
      </Layout>
    </div>
  );
}
